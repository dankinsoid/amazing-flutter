...
sealed class GlassShape {
	...
	static const floatsPerShape = 12;

	/// Packs the shape as (kind, radius, bend, -), (p0, p1), (p2, -, -) — the
	/// layout `liquid_glass.frag` reads.
...
final class GlassCircle extends GlassShape {
	const GlassCircle({required this.center, required this.radius});
...
final class GlassCapsule extends GlassShape {
	const GlassCapsule({required this.a, required this.b, required this.radius});
...
final class GlassRoundedBox extends GlassShape {
	const GlassRoundedBox({required this.rect, required this.cornerRadius});
